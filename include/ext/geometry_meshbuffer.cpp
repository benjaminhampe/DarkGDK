...
///@brief Add MeshBuffer to MeshBuffer
bool addBuffer( SMeshBuffer* dst, SMeshBuffer* src, bool bDropBuffer, scene::E_PRIMITIVE_TYPE primType )
{
	geometry_log("addBuffer()\n");
	...
	/// add BoundingBox
	core::aabbox3df dst_box = dst->getBoundingBox();
	dst_box.addInternalBox( src->getBoundingBox() );
	dst->setBoundingBox( dst_box );

	/// add Vertices
	const u32 vertexCount = src->getVertexCount();
	const u32 fullVertexCount = dst->getVertexCount() + vertexCount;
	for (u32 v = 0; v < vertexCount; v++ )
		dst->Vertices.push_back( src->Vertices[v] );

	/// add Indices
	const u32 indexCount = src->getIndexCount();
	for (u32 i = 0; i < indexCount; i++ )
		dst->Indices.push_back( src->Indices[i] );

	/// drop source buffer, if wished by user ( handy for quick creation of objects )
	if (bDropBuffer)
		src->drop();
...
u32 addVertex( scene::SMeshBuffer* dst, const video::S3DVertex& vertex )
{
	geometry_log( "addVertex()\n");

	if (!dst)
	{
		geometry_error( "Invalid pointer to meshbuffer\n" );
		return 0;
	}

	dst->Vertices.push_back( vertex );

	return dst->Vertices.size();
}
	...
	const video::S3DVertex& b )
{
	...
	// vertices
	u32 vertexCount = dst->Vertices.size();
	dst->Vertices.push_back( a );
	dst->Vertices.push_back( b );

	// indices
	dst->Indices.push_back( vertexCount );
	dst->Indices.push_back( vertexCount+1 );

	return dst->Vertices.size();
}

///@brief get global vertex-count
u32 addVertexTriangle(
	SMeshBuffer* dst,
	video::S3DVertex a,
	video::S3DVertex b,
	video::S3DVertex c )
{
	geometry_log( "addVertexTriangle()\n");
	...
	// vertices
	core::vector3df n = core::plane3df(a.Pos,b.Pos,c.Pos).Normal;
	a.Normal = n;
	b.Normal = n;
	c.Normal = n;
	const u32 vertexCount = dst->Vertices.size();
	dst->Vertices.push_back( a );
	dst->Vertices.push_back( b );
...
{
	geometry_log( "addVertexQuad()\n");

	// vertices
	addVertexTriangle(dst, a,b,c);
	addVertexTriangle(dst, a,c,d);
