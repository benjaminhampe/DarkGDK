...
/*          Code          Bit       Parameter            Range
      ===============     ===  ===================  =============   */
enum S_ERROR {S_YEAR_ERROR,    /*  0   year                  1950 -  2050   */
...
void S_init(struct posdata *pdat)
{
  ...
  pdat->aspect    =  0.0;   /* Azimuth of panel surface (direction it
                                    faces) N=0, E=90, S=180, W=270 */
...
{
    f32 cd;       /* cosine of the declination */
    f32 ch;       /* cosine of the hour angle */
    f32 cl;       /* cosine of the latitude */
    f32 sd;       /* sine of the declination */
    f32 sl;       /* sine of the latitude */
};
...
*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
  ...
  static f32 degrad = 57.295779513; /* converts from radians to degrees */
  static f32 raddeg = 0.0174532925; /* converts from degrees to radians */

/*============================================================================
*    Local function prototypes
============================================================================*/
static s32 validate ( struct posdata *pdat);
...
/*============================================================================
*    s32eger function S_solpos, adapted from the VAX solar libraries
*
...
static s32 validate ( struct posdata *pdat)
{
  ...
  s32 retval = 0;  /* start with no errors */
...
static void geometry ( struct posdata *pdat )
{
  f32 bottom;      /* denominator (bottom) of the fraction */
  f32 c2;          /* cosine of d2 */
  f32 cd;          /* cosine of the day angle or delination */
  f32 d2;          /* pdat->dayang times two */
  f32 delta;       /* difference between current year and 1949 */
  f32 s2;          /* sine of d2 */
  f32 sd;          /* sine of the day angle */
  f32 top;         /* numerator (top) of the fraction */
...
static void zen_no_ref ( struct posdata *pdat, struct trigdata *tdat )
{
  f32 cz;          /* cosine of the solar zenith angle */
...
static void ssha( struct posdata *pdat, struct trigdata *tdat )
{
  f32 cssha;       /* cosine of the sunset hour angle */
  f32 cdcl;        /* ( cd * cl ) */
...
static void sbcf( struct posdata *pdat, struct trigdata *tdat )
{
  f32 p, t1, t2;   /* used to compute sbcf */
...
static void tst( struct posdata *pdat )
{
    ...
    pdat->tstfix =
        ...
        (f32)pdat->hour * 60.0 -
        pdat->minute -
        (f32)pdat->second / 60.0 +
        (f32)pdat->interval / 120.0; /* add back half of the interval */
...
static void sazm( struct posdata *pdat, struct trigdata *tdat )
{
  f32 ca;          /* cosine of the solar azimuth angle */
  f32 ce;          /* cosine of the solar elevation */
  f32 cecl;        /* ( ce * cl ) */
  f32 se;          /* sine of the solar elevation */
...
static void refrac( struct posdata *pdat )
{
  f32 prestemp;    /* temporary pressure/temperature correction */
  f32 refcor;      /* temporary refraction correction */
  f32 tanelev;     /* tangent of the solar elevation angle */
...
{
  f32 ca;          /* cosine of the solar azimuth angle */
  f32 cp;          /* cosine of the panel aspect */
  f32 ct;          /* cosine of the panel tilt */
  f32 sa;          /* sine of the solar azimuth angle */
  f32 sp;          /* sine of the panel aspect */
  f32 st;          /* sine of the panel tilt */
  f32 sz;          /* sine of the refraction corrected solar zenith angle */
...
*    This function decodes the error codes from S_solpos return value
*
*    Requires the s32eger return value from S_solpos
*
...
*           INPUTS:     s32eger S_solpos return value, struct posdata*
...
CSonnenstand nrel_sonnenstand(f64 mJD, f64 geoL, f64 geoB)
{
    ...
    struct nrel::posdata sData;

    //S_init(&sData);

    sData.day = date.getDay();
    sData.daynum = (s32)date.getDayOfYear();
    sData.hour = date.getHour();
    sData.minute = date.getMinute();
    sData.month = date.getMonth();
    sData.second = date.getSecond();
    sData.year = date.getYear();
    sData.interval  =      0;   /* instantaneous measurement interval */
    sData.aspect    =  180.0;   /* Azimuth of panel surface (direction it faces) N=0, E=90, S=180, W=270 */
    sData.latitude  = geoB;   /* Latitude, degrees north (south negative) */
    sData.longitude = geoL;   /* Longitude, degrees east (west negative) */
    sData.press     = 1013.0;   /* Surface pressure, millibars */
    sData.solcon    = 1367.0;   /* Solar constant, 1367 W/sq m */
    sData.temp      =   15.0;   /* Ambient dry-bulb temperature, degrees C */
    sData.tilt      =    0.0;   /* Degrees tilt from horizontal of panel */
    sData.timezone  =  date.getTimezone();   /* Time zone, east (west negative). */
    sData.sbwid     =    7.6;   /* Eppley shadow band width */
    sData.sbrad     =   31.7;   /* Eppley shadow band radius */
    sData.sbsky     =   0.04;   /* Drummond factor for partly cloudy skies */
    sData.function  =  S_ALL;   /* compute all parameters */

    s32 result = nrel::S_solpos (&sData);
	if (result != 0)
	{
		printf("Error in S_solpos() returned.\n");
	}
//    sData.azim-=180.0;
//    while (sData.azim<=360.0)
//    {
//        sData.azim += 360.0;
//    }

    return CSonnenstand(mJD-2.0*core::INV_24, sData.azim, sData.elevref);
}
