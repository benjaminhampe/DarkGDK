...
#ifdef _IRR_COMPILE_WITH_CGUITTFONT_
                          ...
                          const u32& font_size, const FT_Int32& loadFlags)
{
    ...
    core::vector2di page_position((page->used_slots % (texture_side_length / font_size)) * font_size,
    ...
    // we catch glyph bitmap here, so the data won't be removed as soon as the next glyph is loaded.
...
void CGUITTFont::reset_images()
{
    ...
    //always update the internal FT loading flags after resetting
    update_load_flags();
}
...
CGUITTGlyphPage* CGUITTFont::createGlyphPage(const FT_Pixel_Mode& pixel_mode)
{
    ...
    name += size;
    name += "_";
    name += Glyph_Pages.size(); //the newly created page will be at the end of collection.
                      ...
                      bool hcenter, bool vcenter, const core::rect<s32>* clip)
{
    ...
    while (!iter.atEnd())
    {
        ...
        bool visible = ((s32)Invisible.findFirst(currentChar) == -1);
        if (n > 0 && visible)
        {
...
#warning here maybe bug due to endianess of your system
